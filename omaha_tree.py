...
def evaluate(hand_to_evaluate):
    return pokereval.poker_eval(game = "omaha", fill_pockets = 1, iterations = iterations_high, pockets = hand_to_evaluate, dead = [], board = ["__", "__", "__", "__", "__"])['eval'][0]['ev']    

#print str(evalpockets[0]) + ", " + str(evaluate(evalpockets))

with open("converted_hands_evaluation.txt","ab") as out:
    with open("outputfile_new.txt") as omaha_hands:
        for line in omaha_hands:
            line = line.strip('\n')
            newhand = line.split(",")
            newhand.remove("")
            evalpockets = [newhand, emptyhand]
            print str(evalpockets[0]) + ", " + str(evaluate(evalpockets))
